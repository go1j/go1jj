...
$(function () {
        ...
        var car44 = [
            {
                ...
                , num: $('.count-input').val(), price: $('.count-input').val() * 999,
                mingcheng: $('.a1').text(), danjia: $('.qian').text()
            }
        ]
        var car44 = JSON.stringify(car44);
        localStorage.setItem('userCart44', car44)
    ...
    function firstinit(res) {
        // console.log(res)
        // console.log($('.img_box'))
        $('.img_box').html(`<img class="dzp_img" src='${res.b_img4[0]}' alt="">`)
    ...
    function enterinit(res) {
        $(".tupian>ul").on('mouseenter', 'li', function () {
            let imgi = $(this).index();
            $('.img_box').html(`<img class="dzp_img" src='${res.b_img4[imgi]}' alt="">`)
        })
    }
    // 冒泡到小图片上加边框
    $('.tupian>ul').on('mouseenter', 'li', function () {
